...
struct Cli {
    #[command(subcommand)]
    command: Commands,
    #[clap(long, short, default_value = "https://mempool.space/signet/api")]
...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        Commands::SubmitPackage { txs } => {
            let txs: Vec<bitcoin::Transaction> = txs
                .into_iter()
                .map(|s| bitcoin::consensus::encode::deserialize_hex(&s).unwrap())
                .collect();
